...
focus:border-orange-500 focus:outline-none" placeholder="Nhập email" value="<?= isset($_SESSION['user']) ? $email : '' ?>" />
    ...
    <div class="bg-white payment__method flex flex-wrap gap-4 p-5 border border-dashed">
...
">
    ...
    <div class="list__products flex flex-wrap gap-5 justify-center">
      <?php
      ...
      ?>
        <div style="
                ...
                " class="product__checkout w-[310px] flex gap-5 p-2 lg:p-4">
          <div class="product__img bg-white rounded">
            <img src=" <?= $CONTENT_URL ?>/images/products/<?= $item['image'] ?>" alt="" class="w-24 h-24 object-cover rounded-md" />
          </div>
          <div class="product__name flex flex-col justify-between">
            <h3 class="text-lg font-medium t">
              <?= $item['product_name'] ?>
            </h3>
            <p class="text-sm text-gray-500">
              <span class="text-orange-500">Số lượng : </span>
              <?= $item['quantity'] ?>
            </p>
            <p class="text-sm text-gray-500">
              <span class="text-orange-500">Đơn giá : </span>
              <?= number_format($item['price'], 0, ",", ".") ?> đ
            </p>
            <p class="text-sm text-gray-500">
              <span class="text-orange-500">Thành tiền : </span>
...
focus:border-orange-500 focus:outline-none" placeholder="Enter your note"></textarea>
    ...
    <div class="form__group--oder w-full flex flex-col sm:flex-row items-center justify-between gap-3 mt-3 mb-10 lg:mb-20">
      ...
      <button name="order" class="form__input--submit w-1/4  bg-orange-600 text-white p-3 rounded-sm
focus:outline-none">
        Đặt hàng
      </button>
    </div>
