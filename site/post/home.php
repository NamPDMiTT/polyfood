<?php $list_posts = post_select_all();
?>
...
" name="btn_insert" class="flex items-center mt-3 py-2 px-4 rounded-lg text-sm bg-blue-600 text-white shadow-lg">
      ...
      <?php if (isset($_SESSION['user']) && $_SESSION['user']['role_id'] !== 3) { ?>
        <button type="submit" name="btn_check" style="box-shadow: 0 8px 32px 0 rgba(31, 38, 135, 0.37); backdrop-filter: blur(2.5px);
-webkit-backdrop-filter: blur(2.5px); border-radius: 10px; border: 1px solid
rgba(255, 255, 255, 0.18);
" class="flex items-center gap-2 mt-3 py-2 px-4 rounded-lg text-sm bg-red-600 text-white shadow-lg">
  ...
  <?php foreach ($list_posts as $post) : ?>
    ...
    <div class="bg-white shadow-lg rounded-lg mb-6">
      ...
      <?php if (exist_param("btn_check")) { ?>
        <form name="check_posts" action="./index.php" method="POST" class="flex gap-4 items-center justify-center my-5 border-t border-gray-100">
          <button name="btn_confirm" class="flex gap-1 items-center text-xs p-2 rounded-lg  bg-green-500 text-white shadow-lg">
          ...
          <button name="btn_delete" class="flex gap-1 items-center text-xs px-2 py-[6px] rounded-lg  bg-red-600 text-white shadow-lg">
            <span>Xóa</span> <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
  ...
  <?php foreach ($list_posts as $post) : ?>
    ...
    <div class="bg-white shadow-lg rounded-lg mb-6">
      ...
      <div class="text-black p-4 w-full flex">
        <img class="rounded-full h-8 w-8 mr-2 mt-1" src="../../content/images/posts/638867157c6fa_Ảnh chụp màn hình_20221022_024330.png" />
        <div>
          <div class="bg-gray-100 rounded-lg px-4 pt-2 pb-2.5">
            ...
            <div class="text-xs leading-snug md:leading-normal max-w-lg ">
              Lorem ipsum dolor sit amet, consectetur adipisicing elit. Ipsa, sit corporis molestiae at doloribus aliquam assumenda et saepe illo repudiandae, temporibus maiores natus quibusdam sequi. Maxime laboriosam dolorum id quod?
            </div>
            <div class="comment__img my-2">
              <img class="h-20" src="../../content/images/posts/638867157c6fa_Ảnh chụp màn hình_20221022_024330.png" />
            </div>
          </div>
