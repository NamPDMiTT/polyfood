           ...
           <form action="index.php?action=deleteSelectedAccount" method="post">
               <div class="list__accounts-table w-full mt-4">
                   <table class="w-full text-center rounded-md shadow-md my-3">
                       ...
                       <tbody>
                           <!-- Hàm extract() của PHP dùng để lấy key của mảng và gán cho biến -->
                           <?php
                            if (isset($notification)) {
                                echo $notification;
                            }
                            foreach ($accountList as $account) {
                                extract($account); // extract() sẽ tạo ra các biến từ key của mảng - database
                                //Nếu role = 1 thì hiên thị là admin, ngược lại là user
                                $role = $role == 1 ? 'Admin' : 'User';
                                $gender = $gender == 1 ? 'Male' : 'Female';
                                $editAccountUrl = "index.php?action=editAccount&id=" . $id;
                                $deleteAccountUrl = "index.php?action=deleteAccount&id=" . $id;
                           ...
                           <td class="p-2">
                               <img src="../uploads/avatar/' . $avatar . '" alt="" class="w-16 h-16 block mx-auto" />
                           </td>
                           ...
                           <td class="px-2 mt-7 whitespace-nowrap flex gap-3 items-center justify-center">
                               <a href="' . $editAccountUrl . '" class="text-indigo-600 hover:text-indigo-900"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                          <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                      </svg>
                                  </a>
                                  <a href="' . $deleteAccountUrl . '" class="text-indigo-600 hover:text-indigo-900"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
