             ...
             <form action="index.php?menu_id=<?= $menu_id ?>" class="form__add-category w-full" method="post" enctype="multipart/form-data">
                 <div class="list__form-group w-full grid  grid-cols-1 gap-10">
                     <div class="form__group flex flex-col">
                         <label for="categorytid" class="text-gray-500 text-xs sm:text-sm md:text-base lg:text-base">PRODUCT NAME</label>
                         <input placeholder="<?= $product_name ?>" disabled  type="text" name="category_id" id="category_id" class="form__input-add__prodcut shadow-2xl border border-gray-200 focus:outline-none
...
"  />
                     ...
                     <div class="form__group flex flex-col ">
                        <label class="text-xs sm:text-sm md:text-base lg:text-base text-gray-500" for="name">Chọn Sản phẩm thay thế</label>

                        <select name="product_id1" class="mt-2 p-2 px-3  shadow-2xl border border-gray-200 focus:outline-none text-xs sm:text-sm  bg-gray-100 rounded-md text-gray-500">
            <?php foreach($allproducts as $key): ?>
                <?php extract($key) ;?>
             <option value="<?=$product_id?>" <?= $product_id==$check?"selected":"" ?>><?=$product_name?></option>
                <?php endforeach; ?>
          </select>
